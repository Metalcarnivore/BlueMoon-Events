  ...
  if($count>0)
  {
    while($row=mysqli_fetch_assoc($res))
    {
       ...
       if($status1==1)
       {
        ...
        $audience1=$row['audience'];
        $schedule1=$row['datetime'];
		$amount1=$row['amount'];
        $sql2="SELECT * FROM events WHERE bookid='$bookid'";
        ...
        {
            $sql3="INSERT INTO `events`(`bookid`, `name`, `venue_id`, `event_name`, `schedule`, `audience`,`contact`,`amount`) VALUES ('$bookid','$name1','$venue_id1','$event_name1','$schedule1','$audience1','$contact1','$amount1')";
            mysqli_query($conn,$sql3);
        }
      }
      if($status1==2)
      {
        $sql4="DELETE FROM events";
